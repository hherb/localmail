...
def test_a_failure_on_the_dirty_probe_is_named_too(
    ...
    before the dirty probe is reached, so without this the branch ships
    unverified. Needs no real git: the layout satisfies `_repo_is_ours`,
    and the probe is what raises.
    """
    package_dir = tmp_path / "src" / "localmail"
    package_dir.mkdir(parents=True)
