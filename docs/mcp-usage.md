...
| `list_messages` | `account_ids`, `folder_ids`, `limit`, `cursor` | Keyset date-ordered browse (newest first) when you want recent mail narrowed at most by account or folder — "show me recent mail". For any other filter, call `search` without a `query`. |
